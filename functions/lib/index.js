...
Object.defineProperty(exports, "emailSignup", { enumerable: true, get: function () { return contactNotify_1.emailSignup; } });
Object.defineProperty(exports, "onEmailSignupCreated", { enumerable: true, get: function () { return contactNotify_1.onEmailSignupCreated; } });
Object.defineProperty(exports, "onGiveawayEntryCreated", { enumerable: true, get: function () { return contactNotify_1.onGiveawayEntryCreated; } });
...
var firmwareProxy_1 = require("./firmwareProxy");
Object.defineProperty(exports, "firmwareManifest", { enumerable: true, get: function () { return firmwareProxy_1.firmwareManifest; } });
Object.defineProperty(exports, "firmwareBin", { enumerable: true, get: function () { return firmwareProxy_1.firmwareBin; } });
admin.initializeApp({
...
exports.uploadSequence = functions.https.onRequest(async (req, res) => {
    ...
    const { sequenceId, name, category, durationSecs, channelCount, propCount, price, songName, youtubeUrl, youtubeShowUrl, hasMp3, flagged, photos, description, props } = req.body;
...
exports.addToLibrary = functions.https.onCall(async (data, context) => {
    ...
    if (!isFree) {
        throw new functions.https.HttpsError("failed-precondition", "This show is paid. Use Buy to add it to your library after checkout.");
    }
    const purchaseDocRef = db.collection("purchases").doc(`${uid}_${sequenceId}`);
    const existing = await purchaseDocRef.get();
    if (existing.exists) {
...
async function requireAdmin(context) {
    ...
    const emailFromToken = context.auth.token.email || "";
...
exports.ensureSuperAdminClaim = functions.https.onCall(async (_data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Sign-in required.");
    }
    const uid = context.auth.uid;
...
exports.ensureSuperAdminOnCreate = functions.auth.user().onCreate(async (user) => {
    try {
...
exports.submitDmcaNotice = functions.https.onRequest(async (req, res) => {
    ...
    const missing = [];
    if (!claimantName)
        missing.push("claimantName");
    if (!claimantEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(claimantEmail))
        missing.push("claimantEmail");
    if (!claimantPhone)
        missing.push("claimantPhone");
    if (!claimantAddress)
        missing.push("claimantAddress");
    if (!workDescription)
        missing.push("workDescription");
    if (!infringingUrl)
        missing.push("infringingUrl");
    if (!signature)
        missing.push("signature");
    if (!body.attGoodFaith)
        missing.push("attGoodFaith");
    if (!body.attAccuracy)
        missing.push("attAccuracy");
    if (!body.attConsent)
        missing.push("attConsent");
    if (type === "counter" && !body.attJurisdiction)
        missing.push("attJurisdiction");
    if (missing.length) {
...
exports.adminDmcaAction = functions.https.onCall(async (data, context) => {
    const adminUid = await requireAdmin(context);
    ...
    if (action === "forward_to_creator") {
        const counterWindowEnds = admin.firestore.Timestamp.fromMillis(Date.now() + 14 * 24 * 60 * 60 * 1000);
        await noticeRef.update({
            status: "forwarded",
...
exports.userAcceptCreatorAgreement = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Sign-in required.");
    }
    const uid = context.auth.uid;
...
exports.userRequestDataExport = functions.https.onCall(async (_data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Sign-in required.");
    }
    const uid = context.auth.uid;
...
exports.userRequestDataDeletion = functions.https.onCall(async (data, context) => {
    ...
    const scheduledFor = admin.firestore.Timestamp.fromMillis(Date.now() + DATA_DELETION_GRACE_DAYS * 24 * 60 * 60 * 1000);
    await db.collection("users").doc(uid).set({
...
exports.userCancelDeletion = functions.https.onCall(async (_data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Sign-in required.");
    const uid = context.auth.uid;
...
exports.adminRequestUserDeletion = functions.https.onCall(async (data, context) => {
    const adminUid = await requireAdmin(context);
    const uid = data?.uid;
    const reason = typeof data?.reason === "string" ? data.reason.slice(0, 1000) : "";
    if (typeof uid !== "string" || !uid) {
    ...
    for (const doc of snap.docs) {
        ...
        try {
            ...
            try {
                ...
                functions.logger.warn("deleteUser(" + uid + ") failed (already gone?)", e);
            }
            ...
            for (const s of seqs.docs) {
                ...
                if (hasBuyers.empty) {
                    ...
                    await Promise.all([
                        bucket.file("sequences/" + seqId + ".fseq").delete({ ignoreNotFound: true }).catch(() => null),
                        bucket.file("sequences/" + seqId + ".mp3").delete({ ignoreNotFound: true }).catch(() => null),
                    ]);
                    await s.ref.delete();
                }
                else {
...
exports.submitFeedback = functions.https.onCall(async (data, context) => {
    ...
    if (message.length < 3 || message.length > 5000) {
        throw new functions.https.HttpsError("invalid-argument", "Message must be between 3 and 5000 characters.");
    }
    let category = "other";
...
exports.adminReplyFeedback = functions.https.onCall(async (data, context) => {
    const actor = await requireAdmin(context);
    const id = data?.id;
    if (typeof id !== "string" || !isValidId(id)) {
        throw new functions.https.HttpsError("invalid-argument", "id is required.");
...
const SEED_TITLES_SERVER = new Set([
...
const SEED_PACK_IDS = new Set([
    "pack_christmas", "pack_halloween", "pack_patriotic",
]);
function _isSeedSequenceDoc(d) {
    if (!d)
        return false;
    if (d.isSeed === true)
        return true;
    ...
    if (d.source === "seed")
        return true;
    if (d.creatorUid === "system")
        return true;
    const name = typeof d.name === "string" ? d.name : "";
    if (name && SEED_TITLES_SERVER.has(name))
        return true;
    return false;
}
exports.adminPurgeSeedData = functions.https.onCall(async (data, context) => {
    const actor = await requireAdmin(context);
    let seqDeleted = 0;
    let packDeleted = 0;
    ...
    functions.logger.info(`adminPurgeSeedData by ${actor}: ` +
