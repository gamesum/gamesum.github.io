...
function secretsEqual(a: string, b: string): boolean {
  ...
  if (ab.length !== bb.length) {
    try { crypto.timingSafeEqual(ab, ab); } catch {}
    return false;
  }
...
export const purchases = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const purchaseList = snap.docs.map((doc) => {
      const data = doc.data() as PurchaseRecord & { kind?: string };
...
export const getDownloadUrl = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (!existing.exists) {
        const seqDoc = await db.collection("sequences").doc(sequenceId).get();
        const seq = seqDoc.exists ? (seqDoc.data() as { name?: string; creator?: string; creatorUid?: string; price?: number; isFree?: boolean }) : {};
        ...
        if (isFree) {
          await db.runTransaction(async (tx) => {
            ...
            tx.update(db.collection("sequences").doc(sequenceId),
...
function containsVulgarLanguage(text: string): boolean {
...
export const submitReview = functions.https.onCall(async (data, context) => {
  ...
  if (containsVulgarLanguage(comment)) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "Please keep your review clean — that comment was blocked by our content filter."
    );
  }
  ...
  const seqData = seqSnap.data() as { creatorUid?: string };
  ...
  let authorPhotoURL = (context.auth.token as any).picture || "";
  try {
    const userDoc = await db.collection("users").doc(uid).get();
    if (userDoc.exists) {
...
export const deleteOwnReview = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  } catch (err) { functions.logger.warn("rating aggregate update failed", err); }
  return { ok: true };
});
...
export const adminListUsers = functions.https.onCall(async (data, context) => {
  ...
  const users = result.users.map((u) => {
    ...
    return {
      ...
      role,
      photoURL:      u.photoURL || "",
      creatorVerified:           (u.customClaims as any)?.creatorVerified === true || mirror.creatorVerified === true,
      creatorVerificationStatus: (mirror.creatorVerificationStatus as string) || "none",
...
export const exportMyData = functions
  ...
  .https.onCall(async (_data, context) => {
    ...
    try {
      const [
        ...
        db.collection("sequences").where("creatorUid", "==", uid).get(),
        db.collection("creator_earnings").doc(uid).get(),
        db.collection("feedback").where("uid", "==", uid).get().catch(() => ({ docs: [] } as any)),
        email
          ? db.collection("dmca_notices").where("claimantEmail", "==", email).get().catch(() => ({ docs: [] } as any))
          : Promise.resolve({ docs: [] } as any),
      ]);

      const ledgerSnap: any = earningsDoc.exists
        ? await db.collection("creator_earnings").doc(uid).collection("ledger").get()
        : { docs: [] };
      ...
      const payload = {
        ...
        earnings:       earningsDoc.exists ? earningsDoc.data() : null,
        earningsLedger: ledgerSnap.docs.map((d: any) => Object.assign({ id: d.id }, d.data())),
        feedback:       feedbackSnap.docs.map((d: any) => Object.assign({ id: d.id }, d.data())),
        dmcaNotices:    dmcaSnap.docs.map((d: any) => Object.assign({ id: d.id }, d.data())),
      };
      ...
      if (byteLen > MAX_BYTES) {
        throw new functions.https.HttpsError(
          "resource-exhausted",
          "Export exceeds 5MB. Contact support@afterglolighting.org for a full archive."
        );
      }
...
export const adminDeleteFeedback = functions.https.onCall(async (data, context) => {
  const actor = await requireAdmin(context);
  const id = data?.id;
...
export const requestCreatorVerification = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Sign-in required.");
  }
  const uid = context.auth.uid;
...
export const adminSetCreatorVerification = functions.https.onCall(async (data, context) => {
