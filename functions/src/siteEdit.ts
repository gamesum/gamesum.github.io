...
const SITE_EDITOR_GITHUB_TOKEN = defineSecret("SITE_EDITOR_GITHUB_TOKEN");
...
function looksLikeValidHtml(original: string, candidate: string): boolean {
  if (typeof candidate !== "string" || candidate.trim().length === 0) return false;
  const c = candidate.trim().toLowerCase();
  if (!c.startsWith("<!doctype html")) return false;
  if (!c.includes("</html>")) return false;
...
export const siteEdit = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!EDITABLE_FILES.includes(file as any)) {
    ...
    if (!instruction || instruction.length > MAX_INSTRUCTION_LEN) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        `Instruction must be 1-${MAX_INSTRUCTION_LEN} characters.`
      );
    }
    ...
    try {
      ...
      const fileMeta = await githubRequest(
        ghToken,
      ...
      const currentSha = fileMeta.sha as string;

      // 2. Ask Claude to rewrite the file.
      const systemPrompt =
        "You are editing a single HTML file for a live marketing website (AFTERGLO, permanent holiday " +
